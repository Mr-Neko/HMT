...
class TransformerDecoderLayer(Module):
    ...
    def forward(self, input, encoder_output, mask_encoder, pos=None):
        ...
        _, argmax = torch.max(weight_input, dim=1)

        out = outputs[0] * (argmax == 0).unsqueeze(dim=-1).unsqueeze(dim=-1)\
              + outputs[1] * (argmax == 1).unsqueeze(dim=-1).unsqueeze(dim=-1)\
              + outputs[2] * (argmax == 2).unsqueeze(dim=-1).unsqueeze(dim=-1)
