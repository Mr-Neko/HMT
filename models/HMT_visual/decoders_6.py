...
from models.pos3_visual.attention import MultiHeadAttention
...
class DecoderLayer(Module):
    ...
    def forward(self, input, enc_output, mask_pad, mask_self_att, mask_enc_att):
        #MHA+AddNorm
        self_att, _ = self.self_att(input, input, input, None, mask_self_att)
        self_att = self.lnorm1(input + self.dropout1(self_att))
        self_att = self_att * mask_pad
        # MHA+AddNorm
        enc_att, att = self.enc_att(self_att, enc_output, enc_output, None, mask_enc_att)
...
class TransformerDecoderLayer(Module):
    ...
    def forward(self, input, encoder_output, mask_encoder, pos=None):
        ...
        for i, l in enumerate(self.layers):
            out = l(out, encoder_output[5 - i], mask_queries, mask_self_attention, mask_encoder)
