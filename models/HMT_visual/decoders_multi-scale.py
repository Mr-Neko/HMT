...
class DecoderLayer(Module):
    ...
    def forward(self, input, enc_output, mask_pad, mask_self_att, mask_enc_att):
        ...
        # FFN+AddNorm
        ff_layer2 = self.pwff(enc_att_layer2)
        ...
        # FFN+AddNorm
        ff_layer3 = self.pwff(enc_att_layer3)
        ...
        # FFN+AddNorm
        ff_layer4 = self.pwff(enc_att_layer4)
        ff_layer4 = ff_layer4 * mask_pad

        score_layer2 = (ff_layer2 * input).sum(dim=-1)
        score_layer3 = (ff_layer3 * input).sum(dim=-1)
        score_layer4 = (ff_layer4 * input).sum(dim=-1)
