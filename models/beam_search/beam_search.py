...
class BeamSearch(object):
    def __init__(self, model, max_len: int, eos_idx: int, beam_size: int):
        ...
        self.ia = ImageAugmentation

    def _expand_state(self, selected_beam, cur_beam_size):
        def fn(s):

            if s.shape[0] == 3 or s.shape[0] == 6:
                s_list = []
                for temp in s:
                    shape = [int(sh) for sh in temp.shape]
                    beam = selected_beam
                    for _ in shape[1:]:
                        beam = beam.unsqueeze(-1)
                    temp = torch.gather(temp.view(*([self.b_s, cur_beam_size] + shape[1:])), 1,
                                        beam.expand(*([self.b_s, self.beam_size] + shape[1:])))
                    temp = temp.view(*([-1, ] + shape[1:]))

                    s_list.append(temp.unsqueeze(dim=0))
    ...
    def apply(self, visual: list, out_size=1, return_probs=False, **kwargs):
        self.b_s = utils.get_batch_size(visual[2])
        self.device = utils.get_device(visual[2])
    ...
    def iter(self, t: int, visual: list, outputs, return_probs, **kwargs):
        ...
        visuals = self._expand_visual(visual[2], cur_beam_size, selected_beam)
        ...
        return [visual[0], visual[1], visuals], outputs
