...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, relative_pos=None, attention_mask=None, attention_weights=None):
        ...
        R = (torch.abs(indexes.transpose(0, 1).unsqueeze(-1) - indexes.unsqueeze(0)) ** 2).sum(1)
        R = self.W * R.unsqueeze(0)
        # V_ = self.V.expand_as(R)
        # R = torch.div(1 + torch.exp(V_), 1 + torch.exp(V_ - R))
        R = torch.clamp(R*R, 0, 2).unsqueeze(0)

        if keys.shape[1] >= 196:
            R = R.squeeze(0).unsqueeze(-1).view(8, 49, 7, 7)
            R = nn.UpsamplingNearest2d(scale_factor=(2, 2))(R)
            R = R.view(8, 49, 196).unsqueeze(0)
        if keys.shape[1] >= 784:
            R = R.squeeze(0).unsqueeze(-1).view(8, 49, 14, 14)
            R = nn.UpsamplingNearest2d(scale_factor=(2, 2))(R)
            R = R.view(8, 49, 784).unsqueeze(0)
